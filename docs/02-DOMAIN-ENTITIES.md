...
### 17. Attendance
**Purpose**: Records a student's attendance for a given level (`Course`) on a specific date. Stored one row per (level, student, date).

```csharp
public class Attendance : BaseEntity, IAuditableEntity
{
    public Guid CourseId { get; set; }          // the level (FK → Course)
    public Guid StudentId { get; set; }         // FK → User
    public DateOnly Date { get; set; }          // the calendar day
    public AttendanceStatus Status { get; set; }
    public SessionType SessionType { get; set; }
    public Guid RecordedByUserId { get; set; }  // the admin who took it
...
public enum AttendanceStatus
{
    ...
    Late = 2,
    Excused = 3,
    Unexcused = 4,   // i.e. an unexcused absence
    Zoom = 5         // attended remotely
}
...
### 18. ProgressReport
**Purpose**: A Claude-generated weekly progress report. Can be a per-student report (`StudentProgress`) or a per-cohort summary (`ClassSummary`). Generated by the `WeeklyProgressReportJob` (Hangfire) and reviewed/published by an admin. See `docs/04-CLAUDE-REPORTS-ROADMAP.md`.

```csharp
public class ProgressReport : BaseEntity
{
    public Guid? StudentId { get; set; }        // null for ClassSummary reports
    public Guid? CohortId { get; set; }
    ...
    public string? Content { get; set; }        // Claude's markdown narrative
    public string Model { get; set; } = string.Empty;
    public DateTime? GeneratedAt { get; set; }
    public string? FailureReason { get; set; }  // populated when Status = Failed
    public DateTime CreatedAt { get; set; }
...
| Course → Attendance               | 1:N | A level has many attendance records          |
| User → Attendance                 | 1:N | A student has many attendance records        |
| User → ProgressReport             | 1:0..N | A student may have many weekly reports     |
| Cohort → ProgressReport           | 1:0..N | A cohort may have many class-summary reports |
|----------------------------------|------|----------------------------------------------|
...
-- Attendance queries (one record per level/student/day)
CREATE UNIQUE INDEX IX_Attendances_CourseId_StudentId_Date ON Attendances(CourseId, StudentId, Date);
CREATE INDEX IX_Attendances_CourseId_Date ON Attendances(CourseId, Date);
...
// Attendance
- CourseId: Required, FK to Course (the level)
- StudentId: Required, FK to User
- Date: Required (DateOnly)
- Status: Required (Present | Late | Excused | Unexcused | Zoom)
- SessionType: Required (InPerson | Remote)
- RecordedByUserId: Required
- Note: Optional, MaxLength(1000)
- Unique: (CourseId, StudentId, Date) — one record per student per level per day

// ProgressReport
- StudentId: Optional, FK (null for ClassSummary reports)
- CohortId: Optional, FK
- ReportType: Required (StudentProgress | ClassSummary)
- WeekOf: Required
- Status: Required (Pending | Generating | Generated | Failed | Published)
- Model: Required (Claude model id used for generation)
- Content / GeneratedAt / FailureReason: populated as the job progresses
```
