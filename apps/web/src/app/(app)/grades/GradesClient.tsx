...
import { gradesApi, ApiError, type StudentGrades, type Enrollment } from "@/lib/api-client";
...
interface GradesClientProps {
  ...
  initialData: StudentGrades | null;
  initialError: string | null;
}

export function GradesClient({ enrollments, initialCourseId, initialData, initialError }: GradesClientProps) {
  const token = useAuthedToken();
  const [activeCourseId, setActiveCourseId] = useState<string | null>(initialCourseId);
  const [data, setData] = useState<StudentGrades | null>(initialData);
  const [error, setError] = useState<string | null>(initialError);

  // Course switching stays client-side: it fetches through gradesApi, which
  // goes through the /api/proxy route (the browser never holds the JWT).
  const selectCourse = async (courseId: string) => {
    if (courseId === activeCourseId || !token) return;
  ...
  const exportGrades = () => {
    ...
    downloadCsv(`${data.courseName}_Grades.csv`, csvRows);
  };
