...
function StatPill({ icon, label, value, color }: StatPillProps) {
  return (
    <div className={`flex items-center gap-3 rounded-lg border px-4 py-3 ${color}`}>
      ...
      <div>
        <p className="text-xl font-bold leading-none dark:text-slate-100">{value}</p>
        <p className="mt-0.5 text-xs dark:text-slate-400">{label}</p>
      </div>
    </div>
  );
}
...
export function ParticipationCard({ counts, isInstructor }: ParticipationCardProps) {
  ...
  return (
    <div className="rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-5 shadow-sm space-y-4">
      <h2 className="text-sm font-semibold text-gray-900 dark:text-slate-100">Participation</h2>
      ...
      <div className="grid grid-cols-2 gap-3">
        <StatPill
          ...
          color="border-gray-100 dark:border-slate-700 bg-gray-50 dark:bg-slate-900/50 text-gray-700 dark:text-slate-300"
        />
        <StatPill
          icon={<CheckSquare className="h-4 w-4 text-blue-500 dark:text-blue-400" />}
          label="Submitted"
          value={counts.submitted}
          color="border-blue-100 dark:border-blue-900/50 bg-blue-50 dark:bg-blue-950/30 text-blue-700 dark:text-blue-400"
        />
        {isInstructor && counts.needsGrading != null && (
          <StatPill
            ...
            color="border-amber-100 dark:border-amber-900/50 bg-amber-50 dark:bg-amber-950/30 text-amber-700 dark:text-amber-400"
          />
        )}
      </div>
      ...
      {counts.participants > 0 && (
        <div className="space-y-1">
          <div className="flex justify-between text-xs text-gray-500 dark:text-slate-400">
            ...
            <span className="font-medium dark:text-slate-300">{submitPct}%</span>
          </div>
